...
const Itinerary: React.FC<ItineraryModalProps> = ({ isOpen, onClose, packageType }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-2 md:p-4" onClick={onClose}>
      <div 
        className="bg-white rounded-xl md:rounded-2xl max-w-3xl w-full max-h-[95vh] md:max-h-[90vh] overflow-y-auto shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 bg-gradient-to-r from-orange-500 to-orange-600 p-4 md:p-6 rounded-t-xl md:rounded-t-2xl">
          <div className="flex justify-between items-center gap-2">
            <h2 className="text-lg md:text-2xl font-bold text-white">Itinerario - {title}</h2>
            <button 
              onClick={onClose}
              className="text-white hover:bg-white/20 rounded-full p-1.5 md:p-2 transition flex-shrink-0"
            >
        ...
        <div className="p-4 md:p-6 space-y-4 md:space-y-6">
          {itinerary.map((day) => (
            <div key={day.id} className="border-l-4 border-orange-500 pl-3 md:pl-4 py-2">
              <h3 className="text-sm md:text-lg font-bold text-gray-900 mb-1 md:mb-2">{day.title}</h3>
              <p 
                className="text-xs md:text-base text-gray-700 leading-relaxed"
        ...
        {/* Footer */}
        <div className="sticky bottom-0 bg-gray-50 p-4 md:p-6 rounded-b-xl md:rounded-b-2xl border-t">
          <button 
            onClick={onClose}
            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2.5 md:py-3 rounded-lg transition text-sm md:text-base"
          >
            Cerrar
          </button>
        </div>
